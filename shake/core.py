...
def noniter_3p2c(cons):
    ...
    cosectheta = 1/np.sqrt(1 - costheta*costheta)
    d_cot_theta = delta*costheta*cosectheta
    Delta = np.sqrt(d_cot_theta*d_cot_theta + Sigma*Sigma)
    cprime = 0.5*cosectheta*np.sqrt(1 + costheta) 
    sprime = 0.5*cosectheta*np.sqrt(1 - costheta) 
    u1 = np.sqrt(1 + d_cot_theta/Delta)
    u2 = np.sqrt(1 - d_cot_theta/Delta)
    umat = np.array([[u1, -u2], [u2, u1]])
    ctilde = np.array([[cprime, sprime], [-cprime, sprime]])@umat # eq 45
    cmat = np.array([[1/(mu01 * r01_len), 0], [0, 1/(mu0 * r02_len)]])@ctilde
    eigc2 = (Delta - delta*cosectheta)**2
    cminus = 4*Delta*delta*cosectheta
